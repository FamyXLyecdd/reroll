...
async def main():
    ...
    lib.promptAnalytics()
    print()
    # lib.downloadUngoogledChromium() # Skip ungoogled chromium download since we are forcing Brave

    # Automatic Brave Detection
    brave_path = r"C:\Program Files\BraveSoftware\Brave-Browser\Application\brave.exe"
    if os.path.exists(brave_path):
        print(f"Brave Browser detected at {brave_path}")
        print("Automatically using Brave Browser as requested.")
        co.set_browser_path(brave_path)
    else:
        print("Brave Browser not found in default location.")
    ...
    captchaBypass = input(
        ...
        "\nLeave blank for MANUAL solving (no auto-bypass)."
        "\nAPI Key: "
    ).strip()
    ...
    for x in range(int(executionCount)):
        ...
        while captchaPresence and captchaRetries < maxCaptchaRetries:
            ...
            try:
                if captchaBypass != "":
                    print("Setting up NopeCHA with provided API key...")
                    page.get(f"https://nopecha.com/setup#{captchaBypass}")
                else:
                    print("Manual Captcha Mode: You will need to solve any captchas yourself.")
            ...
            if verification is True:
                try:
                    ...
                    if page.ele("@@class=phone-verification-nonpublic-text text-description font-caption-body"):
                        ...
                        await asyncio.sleep(1) # Added wait after input
                        page.ele("@@class=modal-full-width-button btn-primary-md btn-min-width@@text()=Add Email").click()
                        await asyncio.sleep(3) # Increased from 0.5 to ensure modal processes
                        if page.ele("@@class=modal-body@@text()=An email has been sent for verification."):
                            print("Email added (Pending Verification). Skipping verification link fetch as requested.")
                            # link = None
                            # messages = []
                        ...
                        await asyncio.sleep(1) # Added wait
                        page.ele(".modal-button verification-upsell-btn btn-cta-md btn-min-width").click()
                        await asyncio.sleep(3) # Increased to ensure modal process
                    ...
                    # DEBUG: Check email status (Robust)
                    try:
                        with open("debug_email.txt", "a") as f:
                            f.write("Verifying email status in settings...\n")
                            page.get("https://www.roblox.com/my/account#!/info")
                            await asyncio.sleep(5) # Give it good time to load
                            
                            # Save screenshot
                            page.get_screenshot(path="debug_settings.png")
                            f.write("Screenshot saved to debug_settings.png\n")

                            email_label = page.ele("@@class=text-label account-email") 
                            if email_label:
                                f.write(f"Current Email Status (Label): {email_label.text}\n")
                            else:
                                f.write("Could not find simple email label.\n")
                                if email and email in page.html:
                                    f.write(f"Email {email} found in page source.\n")
                                else:
                                    f.write("Email NOT found in page source.\n")
                    except Exception as e:
                        with open("debug_email.txt", "a") as f:
                            f.write(f"Error checking status: {e}\n")
                    
                    # Increased sleep before quit to ensure saves
                    await asyncio.sleep(2)
    ...
    # Auto-save for Account Manager (Feature Request)
    try:
        ram_content = ""
        for account in accounts:
            ...
            if roblosecurityCookie:
                ram_content += f"{roblosecurityCookie}\n"
        
        if ram_content:
            with open("ram_cookies.txt", "a") as f:
                f.write(ram_content)
            print("Saved cookies for Roblox Account Manager to 'ram_cookies.txt'")
    except Exception as e:
        print(f"Error saving RAM cookies: {e}")
    ...
    if accountManagerFormat.lower() in ["y", "yes"]:
        ...
        # Use the already generated content if available, else regenerate (though previous block ensures it matches)
        # Re-using the loop logic for the clipboard to match existing flow
        for account in accounts:
            roblosecurityCookie = None
            for cookie in account["cookies"]:
                if cookie["name"] == ".ROBLOSECURITY":
